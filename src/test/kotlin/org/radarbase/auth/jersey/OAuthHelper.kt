...
class OAuthHelper {
    ...
    init {
        ...
        resource.use { keyStream ->
            ks.load(keyStream, Companion.TEST_KEYSTORE_PASSWORD.toCharArray())
        }

        // get the EC keypair for signing
        val privateKey = ks.getKey(Companion.TEST_SIGNKEY_ALIAS,
                Companion.TEST_KEYSTORE_PASSWORD.toCharArray()) as ECPrivateKey
        val cert = ks.getCertificate(Companion.TEST_SIGNKEY_ALIAS)
    ...
    private fun createValidToken(algorithm: Algorithm): String {
        ...
        return JWT.create()
                ...
                .withSubject(USER)
                .withArrayClaim("scope", Companion.SCOPES)
