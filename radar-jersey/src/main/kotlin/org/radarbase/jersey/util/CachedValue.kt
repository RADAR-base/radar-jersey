 ...
 * not be caught.
 * @param initialValue Initial value of the cache. The value generated by this function is
...
open class CachedValue<T: Any>(
    ...
    protected val state: CacheState
        get() = readLock.locked { CacheState(cache, lastUpdateNanos, _exception) }
    ...
    fun tryRefresh(
        currentUpdateNanos: Long? = null,
        tryLockNanos: Long? = null,
    ): T? = computeSemaphore.tryAcquired(tryLockNanos) {
        doRefresh(currentUpdateNanos)
    }
    ...
    protected inner class CacheState(
        ...
        inline fun <R> shouldRetry(
            valueIsValid: (R) -> Boolean,
            value: R,
        ): Boolean = mayRetry && !valueIsValid(value)
        ...
        inline fun <S> query(method: (T) -> S, valueIsValid: (S) -> Boolean): S = applyValidState(method) {
            if (mustRefresh) {
                val value = tryRefresh(lastUpdateNanos) ?: cache
                method(value)
            } else {
                val result = method(cache)
                if (shouldRetry(valueIsValid, result)) {
                    tryRefresh(lastUpdateNanos)
                        ?.let { method(it) }
                        ?: result
                } else result
            }
        ...
        inline fun get(valueIsValid: (T) -> Boolean): T = applyValidState({ it }) {
            if (mustRefresh || shouldRetry(valueIsValid, cache)) tryRefresh(lastUpdateNanos) ?: cache
            else cache
        }
